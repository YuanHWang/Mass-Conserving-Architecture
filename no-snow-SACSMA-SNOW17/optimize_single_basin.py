...
def run_single_basin(basin, train_dates, algorithm, max_model_runs, dds_trials, out_dir_run):
    ...
    sd = train_dates['start_dates']
    ed = train_dates['end_dates']
    #ed = train_dates['end_dates'][basin]
    obj_fun_dates = pd.DataFrame(list(pd.date_range(sd, ed)), columns = ('train_dates',))
    #obj_fun_dates = pd.DataFrame(list(chain.from_iterable(pd.date_range(sdi, edi) for sdi, edi in zip(sd, ed))), columns = ('train_dates',))

    skillflag = pd.read_csv('leafriver_trainflag.txt', header=None, delimiter=r"\s+")
    skillflag = skillflag.rename(columns={0: 'flag'})
    mask = obj_fun_dates['train_dates'][skillflag['flag']==-1]
    #print(mask.shape)
    # load data
    mask_dates = mask #obj_fun_dates['train_dates']
    #print(mask_dates.shape)    
...
#    print(area)
    ...
    #outfile = out_dir_run + str(basin) + '.csv'
