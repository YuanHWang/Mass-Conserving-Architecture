...
#        self.temperature = 0.5 * (forcings['Tmax(C)'].values + forcings['Tmin(C)'].values)
        # Assign temperature to be exactly equals to precipitation, the purpose is to pass dummy time series into the function. temperature does not really being used here
        self.temperature = self.precipitation #0.5 * (forcings['Tmax(C)'].values + forcings['Tmin(C)'].values)
        ...
        self.pet = forcings['PET(mm/day)'].values#priestley_taylor_pet(forcings['Tmin(C)'], forcings['Tmax(C)'], forcings['SRAD(W/m2)'],
...
#                                   forcings.index.to_series().dt.dayofyear)
    ...
    def _run(self, test_parms, test_param_names):
        ...
        for t in range(self.precipitation.shape[0]):
            ...
            surf, grnd, qq[t], tet = sacsma.exsac(self.dt_seconds, self.precipitation[t], self.temperature[t], self.pet[t],
        ...
        #                                         1, 0)
        #print(qq.shape)
        #print(hydrograph_qq.shape)
        # create output series
        simulated_hydrograph_series = pd.Series(qq, index=self.dates)
        #simulated_hydrograph_series = pd.Series(hydrograph_qq, index=self.dates)
