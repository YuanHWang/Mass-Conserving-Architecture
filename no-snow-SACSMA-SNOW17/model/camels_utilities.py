...
# Hard-code paths
DATA_DIR = '/xdisk/tyferre/yhwang0730/CAMELS_US'
...
def load_all_sacsma_parameters(forcing_type: str):
  ...
  filenames = glob.glob(f'{DATA_DIR}/model_output_no_snow/{forcing_type}/*_model_parameters.txt')
...
def load_sacsma_parameters(gauge_id: str):
  ...
  filename = glob.glob(f'{DATA_DIR}/model_output_no_snow/{FORCING_TYPE}/{gauge_id}_*_model_parameters.txt')[0]
...
def load_forcings(gauge_id: str):
  ...
  forcing_files = glob.glob(
      ...
      f'{DATA_DIR}/basin_mean_forcing/{FORCING_TYPE}/**/{gauge_id}_*_forcing_leap.txt')
  #print(len(forcing_files))
  #print(forcing_files)
...
def load_discharge(gauge_id: str):
  ...
  filename = glob.glob(f'{DATA_DIR}/model_output_no_snow/{FORCING_TYPE}/{gauge_id}_*_model_output.txt')[0]
...
def load_usgs(gauge_id: str, area: int):
  ...
  filename = glob.glob(f'{DATA_DIR}/usgs_streamflow/**/{gauge_id}_streamflow_qc.txt')[0]
  ...
  # Annotate out to run leaf river data
