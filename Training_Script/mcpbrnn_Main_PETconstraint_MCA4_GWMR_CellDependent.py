...
import csv
# import class
from MCPBRNN_lib_tools.HyMODZoo import MCPBRNN_Generic_PETconstraint_Two_VariantOutputGate, MCPBRNN_GWVariant_Routing,MCPBRNN_GWVariant_Routing_MRRegular_Relaxed
...
CaseName = 'MCPBRNN_Generic_PETconstraint_MCA4_GWMR_CellDP_NoPARConstraint_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"])
directory = CaseName
...
par_no = 17 # Temporarily not save the parameter value
skillmetric = 7
...
class Model(nn.Module):
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd, GW_mean, GW_std, Ini_C):
        ...
        last_h = self.dropout(h_t+h_t_MCA4)      
        out = last_h        
        return out, h_t, h_t_MCA4, c_t, c_t_MCA4, l_t, lc_t, gw_t, bp_t, i, oo, oo_MCA4, ol, olc, f, f_MCA4, oogw, ov_MCA4
...
for epoch in range(1,num_epochs+1): 
    ...
    for data in pbar:
        ...
        predictions = model(x,epoch, time_lag, y, cmean, cstd, GW_mean, GW_std, Ini_C)[0]      
    ...
    Result = model(x,epoch, time_lag, y, cmean, cstd, GW_mean, GW_std, Ini_C)
    out_eval = Result[0]
    hidden_eval = Result[1] 
    hidden_MCA4_eval = Result[2]        
    cell_eval = Result[3]
    cell_MCA4_eval = Result[4]    
    loss_eval = Result[5]
    lossc_eval = Result[6]    
    recharge_eval = Result[7]    
    bypass_eval = Result[8]
    i_gate = Result[9]
    o_gate = Result[10]
    o_gate_MCA4 = Result[11]    
    l_gate = Result[12] 
    lc_gate = Result[13]          
    f_gate = Result[14]
    f_gate_MCA4 = Result[15]
    ogw_gate = Result[16] 
    ov_gate_MCA4 = Result[17] 
    ...
    OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode_GW.weight_r_yom"][:]
    OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode_GW.weight_r_yfm"][:]
    OutMatrix[epoch-1,12]= model.state_dict()["MCPBRNNNode_GW.weight_r_yvm"][:]
    OutMatrix[epoch-1,13]= model.state_dict()["MCPBRNNNode_GW.bias_b0_yom"][:]        
    OutMatrix[epoch-1,14]= model.state_dict()["MCPBRNNNode_GW.weight_b1_yom"][:]
    OutMatrix[epoch-1,15]= model.state_dict()["MCPBRNNNode_GW.bias_b0_yrm"][:]
    OutMatrix[epoch-1,16]= model.state_dict()["MCPBRNNNode_GW.weight_s_yvm"][:]
