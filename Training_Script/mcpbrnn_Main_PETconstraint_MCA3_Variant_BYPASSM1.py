...
import csv
# import class
from MCPBRNN_lib_tools.NodeZoo import MCPBRNN_Generic_PETconstraint_Scaling, MCPBRNN_Generic_PETconstraint_Scaling_BYPASSM1
from MCPBRNN_lib_tools.HyMODZoo import MCPBRNN_SW_Variant_Routing
...
CaseName = 'MCPBRNN_Generic_PETconstraint_MCA3_Variant_BYPASSM1_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"])
directory = CaseName
...
par_no = 13 # Temporarily not save the parameter value
skillmetric = 7
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.dropout_rate = dropout       
        self.batch_size = batch_size        
        self.MCPBRNNNode = MCPBRNN_Generic_PETconstraint_Scaling_BYPASSM1(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
...
weight_file = "MCA3/MCPBRNN_Generic_PETconstraint_MCA3_Variant_1Layer_1node_9/model_epoch193.pt"
...
for epoch in range(1,num_epochs+1): 
    ...
    for data in pbar:
        ...
        predictions = model(x,epoch, time_lag, y, c_mean, c_std,SW_mean,SW_std)[0]      
    ...
    Result = model(x,epoch, time_lag, y, c_mean, c_std, SW_mean, SW_std)
    out_eval = Result[0]
    hidden_eval = Result[1]
    cell_sw_eval = Result[2]
    cell_eval = Result[3]    
    loss_eval = Result[4]
    lossc_eval = Result[5]    
    bypass_eval = Result[6]
    i_gate = Result[7]
    o_sw_gate = Result[8]
    o_gate = Result[9]
    l_gate = Result[10]
    ...
    cell_sw_eval = cell_sw_eval.detach().cpu().numpy()
    ...
    OutMatrix[epoch-1,7]= model.state_dict()["MCPBRNNNode.bias_b0_yum"][:] 
    OutMatrix[epoch-1,8]= model.state_dict()["MCPBRNNNode.weight_b1_yum"][:]            
    OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode_SW.weight_r_yom"][:]
    OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode_SW.weight_r_yfm"][:]
    OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode_SW.bias_b0_yom"][:]
    OutMatrix[epoch-1,12]= model.state_dict()["MCPBRNNNode_SW.weight_b1_yom"][:]
