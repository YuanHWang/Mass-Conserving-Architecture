...
from MCPBRNN_lib_tools.HyMODZoo import  MCPBRNN_GWVariant_Routing_MRRegular, MCPBRNN_Generic_PETconstraint_Two_VariantOutputGate, MCPBRNN_GWVariant_Routing, MCPBRNN_SW_Variant_Routing,MCPBRNN_Generic_PETconstraint_Three_VariantOutputGate
...
parser.add_argument('--SW_mean',
                        type=float,
                        default=3.051248249,
                        help="")

parser.add_argument('--SW_std',
                        type=float,
                        default=3.365261828,
                        help="")

parser.add_argument('--GW_mean',
                        type=float,
                        default=1.87176748,
                        help="")

parser.add_argument('--GW_std',
                        type=float,
                        default=2.085571043,
                        help="")

parser.add_argument('--Ini_C',
                        type=float,
                        default=1.6871063,
                        help="")
...
CaseName = 'MCPBRNN_Generic_PETconstraint_MCA6_CellDP_YesPARConstraint_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"])
directory = CaseName
...
par_no = 24 # Temporarily not save the parameter value
skillmetric = 7
...
class Model_old(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode = MCPBRNN_Generic_PETconstraint_Three_VariantOutputGate(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
                         initial_forget_bias=initial_forget_bias)
        self.MCPBRNNNode_SW = MCPBRNN_SW_Variant_Routing(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
                         initial_forget_bias=initial_forget_bias)  
        self.MCPBRNNNode_GW = MCPBRNN_GWVariant_Routing(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd, SW_mean, SW_std, GW_mean, GW_std, Ini_C):

        h_t, h_fp_t, c_t, l_t, lc_t, bp_t, gw_t, i, oo, oofp, ol, olc, f, oogw, h_nout, obs_std = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval, cmean, cstd)     
        h_t_MCA3, c_t_MCA3, oo_MCA3, f_MCA3 = self.MCPBRNNNode_SW(h_t, epoch, time_lag, y_eval, SW_mean, SW_std) 
        h_t_MCA4, c_t_MCA4, oo_MCA4, f_MCA4 = self.MCPBRNNNode_GW(gw_t, epoch, time_lag, y_eval, GW_mean, GW_std, Ini_C) 
        last_h = self.dropout(h_fp_t+h_t_MCA3+h_t_MCA4)      
        out = last_h        
        
        return out, h_t, h_t_MCA3, h_t_MCA4, c_t, c_t_MCA3, c_t_MCA4, l_t, lc_t, gw_t, bp_t, i, oo, oo_MCA3, oo_MCA4, ol, olc, f, f_MCA3, f_MCA4, oogw, h_fp_t, oofp

model_old = Model_old(input_size_dyn=input_size_dyn,
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode_SW = MCPBRNN_SW_Variant_Routing(input_size=input_size_dyn,
                         ...
                         traintimeLen=traintimeLen,              
                         initial_forget_bias=initial_forget_bias)  
        self.MCPBRNNNode_GW = MCPBRNN_GWVariant_Routing_MRRegular(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd, SW_mean, SW_std, GW_mean, GW_std, Ini_C):
        ...
        h_t, h_fp_t, c_t, l_t, lc_t, bp_t, gw_t, i, oo, oofp, ol, olc, f, oogw, h_nout, obs_std = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval, cmean, cstd)     
        h_t_MCA3, c_t_MCA3, oo_MCA3, f_MCA3 = self.MCPBRNNNode_SW(h_t, epoch, time_lag, y_eval, SW_mean, SW_std) 
        h_t_MCA4, c_t_MCA4, oo_MCA4, f_MCA4, ov_MCA4 = self.MCPBRNNNode_GW(gw_t, epoch, time_lag, y_eval, GW_mean, GW_std, Ini_C) 
        last_h = self.dropout(h_fp_t+h_t_MCA3+h_t_MCA4)      
        out = last_h        
        
        return out, h_t, h_t_MCA3, h_t_MCA4, c_t, c_t_MCA3, c_t_MCA4, l_t, lc_t, gw_t, bp_t, i, oo, oo_MCA3, oo_MCA4, ol, olc, f, f_MCA3, f_MCA4, oogw, h_fp_t, oofp, ov_MCA4
...
weight_file = "MCA6/MCPBRNN_Generic_PETconstraint_MCA6_1Layer_1node_2/model_epoch469.pt"
model_old.load_state_dict(torch.load(weight_file))

model.state_dict()["MCPBRNNNode.weight_r_yom"][:] = model_old.state_dict()["MCPBRNNNode.weight_r_yom"][:]
model.state_dict()["MCPBRNNNode.weight_r_yom_gw"][:] = model_old.state_dict()["MCPBRNNNode.weight_r_yom_gw"][:]
model.state_dict()["MCPBRNNNode.weight_r_yom_fp"][:] = model_old.state_dict()["MCPBRNNNode.weight_r_yom_fp"][:]
model.state_dict()["MCPBRNNNode.weight_r_ylm"][:] = model_old.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
model.state_dict()["MCPBRNNNode.weight_r_yfm"][:] = model_old.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
model.state_dict()["MCPBRNNNode.bias_b0_yom"][:] = model_old.state_dict()["MCPBRNNNode.bias_b0_yom"][:]
model.state_dict()["MCPBRNNNode.weight_b1_yom"][:] = model_old.state_dict()["MCPBRNNNode.weight_b1_yom"][:]
model.state_dict()["MCPBRNNNode.bias_b0_yom_gw"][:] = model_old.state_dict()["MCPBRNNNode.bias_b0_yom_gw"][:]
model.state_dict()["MCPBRNNNode.weight_b1_yom_gw"][:] = model_old.state_dict()["MCPBRNNNode.weight_b1_yom_gw"][:]
model.state_dict()["MCPBRNNNode.bias_b0_ylm"][:] = model_old.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]
model.state_dict()["MCPBRNNNode.weight_b2_ylm"][:] = model_old.state_dict()["MCPBRNNNode.weight_b2_ylm"][:]

model.state_dict()["MCPBRNNNode_SW.weight_r_yom"][:] = model_old.state_dict()["MCPBRNNNode_SW.weight_r_yom"][:]
model.state_dict()["MCPBRNNNode_SW.weight_r_yfm"][:] = model_old.state_dict()["MCPBRNNNode_SW.weight_r_yfm"][:]
model.state_dict()["MCPBRNNNode_SW.bias_b0_yom"][:] = model_old.state_dict()["MCPBRNNNode_SW.bias_b0_yom"][:]
model.state_dict()["MCPBRNNNode_SW.weight_b1_yom"][:] = model_old.state_dict()["MCPBRNNNode_SW.weight_b1_yom"][:]

model.state_dict()["MCPBRNNNode_GW.weight_r_yom"][:] = model_old.state_dict()["MCPBRNNNode_GW.weight_r_yom"][:]
model.state_dict()["MCPBRNNNode_GW.weight_r_yfm"][:] = model_old.state_dict()["MCPBRNNNode_GW.weight_r_yfm"][:]
model.state_dict()["MCPBRNNNode_GW.bias_b0_yom"][:] = model_old.state_dict()["MCPBRNNNode_GW.bias_b0_yom"][:]
model.state_dict()["MCPBRNNNode_GW.weight_b1_yom"][:] = model_old.state_dict()["MCPBRNNNode_GW.weight_b1_yom"][:]
...
for epoch in range(1,num_epochs+1):
    ...
    for data in pbar:
        ...
        predictions = model(x,epoch, time_lag, y, cmean, cstd, SW_mean, SW_std, GW_mean, GW_std, Ini_C)[0]      
    ...
    Result = model(x,epoch, time_lag, y, cmean, cstd, SW_mean, SW_std, GW_mean, GW_std, Ini_C)
    out_eval = Result[0]
    ...
    loss_eval = Result[7]
    lossc_eval = Result[8]    
    recharge_eval = Result[9]    
    bypass_eval = Result[10]
    i_gate = Result[11]
    o_gate = Result[12]
    o_gate_MCA3 = Result[13]    
    o_gate_MCA4 = Result[14]    
    l_gate = Result[15] 
    lc_gate = Result[16]          
    f_gate = Result[17]
    f_gate_MCA3 = Result[18]
    f_gate_MCA4 = Result[19]
    ogw_gate = Result[20] 
    hidden_fp_eval = Result[21] 
    o_gate_fp = Result[22]     
    ov_gate_MCA4 = Result[23] 
    ...
    hidden_fp_eval = hidden_fp_eval.detach().cpu().numpy()
    o_gate_fp = o_gate_fp.detach().cpu().numpy()   
    ov_gate_MCA4 = ov_gate_MCA4.detach().cpu().numpy() 
    #calculate the skill
...
# Output time series
    '''Out_out = out_eval[:,0]  
    Out_hidden = hidden_eval[:,0]       
    Out_hidden_SW = hidden_MCA3_eval[:,0]
    Out_hidden_GW = hidden_MCA4_eval[:,0]
    Out_cell = cell_eval[:,0]   
    Out_cell_SW  = cell_MCA3_eval[:,0]  
    Out_cell_GW  = cell_MCA4_eval[:,0]   
    Out_loss = loss_eval[:,0]   
    Out_lossc = lossc_eval[:,0]   
    Out_recharge = recharge_eval[:,0]   
    Out_o_gate = o_gate[:,0]   
    Out_o_gate_SW = o_gate_MCA3[:,0]  
    Out_o_gate_GW = o_gate_MCA4[:,0]   
    Out_l_gate = l_gate[:,0]   
    Out_lc_gate = lc_gate[:,0]   
    Out_f_gate = f_gate[:,0]   
    Out_f_gate_SW = f_gate_MCA3[:,0]  
    Out_f_gate_GW = f_gate_MCA4[:,0]   
    Out_ogw_gate = ogw_gate[:,0]'''
    ...
    OutMatrix[epoch-1,2]= model.state_dict()["MCPBRNNNode.weight_r_yom_fp"][:]    
    OutMatrix[epoch-1,3]= model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
    OutMatrix[epoch-1,4]= model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
    OutMatrix[epoch-1,5]= model.state_dict()["MCPBRNNNode.bias_b0_yom"][:]
    OutMatrix[epoch-1,6]= model.state_dict()["MCPBRNNNode.weight_b1_yom"][:]
    OutMatrix[epoch-1,7]= model.state_dict()["MCPBRNNNode.bias_b0_yom_gw"][:]
    OutMatrix[epoch-1,8]= model.state_dict()["MCPBRNNNode.weight_b1_yom_gw"][:]
    OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode.bias_b0_yom_fp"][:]
    OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode.weight_b1_yom_fp"][:]    
    OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]
    OutMatrix[epoch-1,12]= model.state_dict()["MCPBRNNNode.weight_b2_ylm"][:]
    OutMatrix[epoch-1,13]= model.state_dict()["MCPBRNNNode_SW.weight_r_yom"][:]
    OutMatrix[epoch-1,14]= model.state_dict()["MCPBRNNNode_SW.weight_r_yfm"][:]
    OutMatrix[epoch-1,15]= model.state_dict()["MCPBRNNNode_SW.bias_b0_yom"][:]
    OutMatrix[epoch-1,16]= model.state_dict()["MCPBRNNNode_SW.weight_b1_yom"][:]
    OutMatrix[epoch-1,17]= model.state_dict()["MCPBRNNNode_GW.weight_r_yom"][:]
    OutMatrix[epoch-1,18]= model.state_dict()["MCPBRNNNode_GW.weight_r_yfm"][:]
    OutMatrix[epoch-1,19]= model.state_dict()["MCPBRNNNode_GW.bias_b0_yom"][:]
    OutMatrix[epoch-1,20]= model.state_dict()["MCPBRNNNode_GW.weight_b1_yom"][:]
    OutMatrix[epoch-1,21]= model.state_dict()["MCPBRNNNode_GW.weight_r_yvm"][:]
    OutMatrix[epoch-1,22]= model.state_dict()["MCPBRNNNode_GW.weight_s_yvm"][:]
    OutMatrix[epoch-1,23]= model.state_dict()["MCPBRNNNode_GW.bias_b0_yrm"][:]

    print(B2)
    torch.save(model.state_dict(), savetext)


outtofile = pd.DataFrame(OutMatrix, columns=["MCPBRNNNode.weight_r_yom","MCPBRNNNode.weight_r_yom_gw","MCPBRNNNode.weight_r_yom_fp","MCPBRNNNode.weight_r_ylm","MCPBRNNNode.weight_r_yfm","MCPBRNNNode.bias_b0_yom","MCPBRNNNode.weight_b1_yom","MCPBRNNNode.bias_b0_yom_gw","MCPBRNNNode.weight_b1_yom_gw","MCPBRNNNode.bias_b0_yom_fp","MCPBRNNNode.weight_b1_yom_fp","MCPBRNNNode.bias_b0_ylm","MCPBRNNNode.weight_b2_ylm","MCPBRNNNode_SW.weight_r_yom","MCPBRNNNode_SW.weight_r_yfm","MCPBRNNNode_SW.bias_b0_yom","MCPBRNNNode_SW.weight_b1_yom","MCPBRNNNode_GW.weight_r_yom","MCPBRNNNode_GW.weight_r_yfm","MCPBRNNNode_GW.bias_b0_yom","MCPBRNNNode_GW.weight_b1_yom","MCPBRNNNode_GW.weight_r_yvm","MCPBRNNNode_GW.weight_s_yvm","MCPBRNNNode_GW.bias_b0_yrm",'NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])
...
outname = CaseName + '/' + 'Outrecharge_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)
