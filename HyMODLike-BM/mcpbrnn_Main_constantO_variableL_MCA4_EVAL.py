...
from MCPBRNN_lib_tools.HyMODZoo import MCPBRNN_Generic_constantoutput_variableLoss_MCA2, MCPBRNN_GWConstant_Routing
...
CaseName = 'MCPBRNN_Generic_M2' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"]) + '_MCA4'
directory = CaseName
...
Out_hidden_M4 = np.zeros((timeLen,1))
Out_cell = np.zeros((timeLen,1))
Out_cell_M4 = np.zeros((timeLen,1))
...
Out_o_M4_gate = np.zeros((timeLen,1))
Out_l_gate = np.zeros((timeLen,1))
Out_gw_gate = np.zeros((timeLen,1))
Out_f_gate = np.zeros((timeLen,1))
Out_f_M4_gate = np.zeros((timeLen,1))
...
for epoch in range(1,1+1): 
    ...
    bypass_eval = Result[7]
    i_gate = Result[8]
    o_gate = Result[9]
    o_gate_MCA4 = Result[10]    
    ogw_gate = Result[11]    
    l_gate = Result[12]
    f_gate = Result[13]
    f_gate_MCA4 = Result[14]
...
# Output time series
    ...
    Out_hidden_M4[0:timeLen,epoch-1] = hidden_MCA4_eval[:,0]           
    Out_cell[0:timeLen,epoch-1] = cell_eval[:,0]
    Out_cell_M4[0:timeLen,epoch-1] = cell_MCA4_eval[:,0]    
    ...
    Out_o_M4_gate [0:timeLen,epoch-1] = o_gate_MCA4[:,0]    
    Out_l_gate [0:timeLen,epoch-1] = l_gate[:,0]    
    Out_gw_gate [0:timeLen,epoch-1] = ogw_gate[:,0]
    Out_f_gate [0:timeLen,epoch-1] = f_gate[:,0]
    Out_f_M4_gate [0:timeLen,epoch-1] = f_gate_MCA4[:,0]    

    print(B2)

CaseName = "results_temp/M2-case-MCA4/"
# Save timeseries
...
Out_file = pd.DataFrame(Out_hidden_M4)
...
Out_file = pd.DataFrame(Out_cell_M4)
...
Out_file = pd.DataFrame(Out_o_M4_gate)
...
Out_file = pd.DataFrame(Out_f_M4_gate)
outname = CaseName + '/' + 'Out_gatef_GW_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_gw_gate)
outname = CaseName + '/' + 'Out_gategw_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)