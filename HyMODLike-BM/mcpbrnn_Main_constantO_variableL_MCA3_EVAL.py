...
Out_o_sw_gate = np.zeros((timeLen,1))
Out_l_gate = np.zeros((timeLen,1))
Out_f_gate = np.zeros((timeLen,1))
Out_f_sw_gate = np.zeros((timeLen,1))
...
# Output time series
    ...
    Out_o_sw_gate [0:timeLen,epoch-1] = o_sw_gate[:,0]    
    Out_l_gate [0:timeLen,epoch-1] = l_gate[:,0]    
    Out_f_gate [0:timeLen,epoch-1] = f_gate[:,0]
    Out_f_sw_gate [0:timeLen,epoch-1] = f_sw_gate[:,0]    

    print(B2)

CaseName = "results_temp/M2-case-MCA3/"
# Save timeseries
...
Out_file = pd.DataFrame(Out_o_sw_gate)
...
Out_file = pd.DataFrame(Out_f_sw_gate)
