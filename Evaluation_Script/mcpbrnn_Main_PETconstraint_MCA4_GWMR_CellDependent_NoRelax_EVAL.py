...
Out_mr_gate = np.zeros((timeLen,1))
...
class Model(nn.Module):
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd, GW_mean, GW_std, Ini_C):
        ...
        out = last_h        
        return out, h_t, h_t_MCA4, c_t, c_t_MCA4, l_t, lc_t, gw_t, bp_t, i, oo, oo_MCA4, ol, olc, f, f_MCA4, oogw, ov_MCA4, MRF
...
weight_file = "results_HYMOD/MCA4-GWMR-celldep-Yesconstraint/MCPBRNN_Generic_PETconstraint_MCA4_GWMR_CellDP_YesPARConstraint_1Layer_1node_4/model_epoch1996.pt"
...
# Output time series
    ...
    Out_mr_gate = ov_gate_MCA4[:,0]   
    Out_MRF = MRF[:,0]
    print(B2)

# Save Results file

CaseName = 'results_HYMOD/MCA4-GWMR-celldep-Yesconstraint/'
...
Out_file = pd.DataFrame(Out_mr_gate)
outname = CaseName + '/' + 'Out_gate_mr_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)
