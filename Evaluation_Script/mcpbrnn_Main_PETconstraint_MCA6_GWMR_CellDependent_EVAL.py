...
Out_hidden_FP = np.zeros((timeLen,1))
...
Out_o_gate_FP = np.zeros((timeLen,1))
...
class Model(nn.Module):
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd, SW_mean, SW_std, GW_mean, GW_std, Ini_C):
        ...
        h_t_MCA4, c_t_MCA4, oo_MCA4, f_MCA4, ov_MCA4,MRF = self.MCPBRNNNode_GW(gw_t, epoch, time_lag, y_eval, GW_mean, GW_std, Ini_C) 
        last_h = self.dropout(h_fp_t+h_t_MCA3+h_t_MCA4)      
        out = last_h        
        
        return out, h_t, h_t_MCA3, h_t_MCA4, c_t, c_t_MCA3, c_t_MCA4, l_t, lc_t, gw_t, bp_t, i, oo, oo_MCA3, oo_MCA4, ol, olc, f, f_MCA3, f_MCA4, oogw, h_fp_t, oofp, ov_MCA4,MRF
...
weight_file = "results_HYMOD/MCA6-GWMR-celldep-Noconstraint/MCPBRNN_Generic_PETconstraint_MCA6_CellDP_NoPARConstraint_1Layer_1node_2/model_epoch507.pt"
...
# Output time series
    ...
    Out_hidden_FP = hidden_fp_eval[:,0]    
...
CaseName = 'results_HYMOD/MCA6-GWMR-celldep-Noconstraint/'
...
Out_file = pd.DataFrame(Out_hidden_FP)
outname = CaseName + '/' + 'Outhidden_FP_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)
...
Out_file = pd.DataFrame(Out_o_gate_FP)
outname = CaseName + '/' + 'Out_gateo_FP_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)
...
Out_file = pd.DataFrame(Out_ov_gate)
outname = CaseName + '/' + 'Out_gateov_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_MRF)
outname = CaseName + '/' + 'Out_MRF_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)