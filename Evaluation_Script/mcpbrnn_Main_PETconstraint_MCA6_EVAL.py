...
Out_quickflow = np.zeros((timeLen,1))
Out_fp_gate = np.zeros((timeLen,1))
...
weight_file = "results_HYMOD/MCA6/MCPBRNN_Generic_PETconstraint_MCA6_1Layer_1node_2/model_epoch469.pt"
...
# Output time series
    ...
    Out_quickflow = hidden_fp_eval[:,0]
    Out_fp_gate = o_gate_fp[:,0]

    print(B2)

CaseName = 'results_HYMOD/MCA6/'
...
Out_file = pd.DataFrame(Out_quickflow)
outname = CaseName + '/' + 'Out_quickflow_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_fp_gate)
outname = CaseName + '/' + 'Out_fp_gate_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)