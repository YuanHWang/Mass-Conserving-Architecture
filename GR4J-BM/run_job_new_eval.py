...
def gr4j(precip, potential_evap, qobs, params, states = None, return_state = False):
    """
...
# Output Evaluation
outfile = CaseName + '/GR4J-Test_BestPar_' + str(Trial_no) + '.csv' 
params = pd.read_csv(outfile)
sim, obs = gr4j(PP, PET, qobs, params, states = None, return_state = False)
output = pd.DataFrame(np.concatenate((sim,obs), axis=1),columns=['sim','obs'])
print(output)
outfile = CaseName + '/GR4J-Test_timeseries_' + str(Trial_no) + '.csv'
